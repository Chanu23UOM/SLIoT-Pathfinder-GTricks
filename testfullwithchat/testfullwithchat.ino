...
<body>
  ...
  <script>
    ...
    // --- BUTTON TRIGGERS (DEBUGGED) ---
    // Removed confirm() popup as it is blocked by mobile Captive Portals.
    // Added a timestamp '?t=' to prevent the browser from caching the click.
    document.getElementById('sosBtn').addEventListener('click', function() {
      const btn = document.getElementById('sosBtn');
      btn.innerText = "TRANSMITTING...";
      btn.style.backgroundColor = "#B71C1C"; // Darker red while sending
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("\n--- Initializing Pathfinder Node ---");
  
  if (!mpu.begin()) { 
    Serial.println("SYSTEM WARNING: MPU6050 unavailable. Initiating mock seismic mode."); 
    hasMPU = false;
  } else { 
  ...
  if (!bme.begin(0x76)) { 
    Serial.println("SYSTEM WARNING: BME280 unavailable. Initiating mock telemetry mode."); 
    hasBME = false;
  } else { hasBME = true; }

  LoRa.setPins(LORA_SS, LORA_RST, LORA_DIO0);
  if (!LoRa.begin(433E6)) { 
    Serial.println("SYSTEM WARNING: LoRa module failed. Transmissions routed to Serial only."); 
    hasLoRa = false;
  } else { hasLoRa = true; }
  ...
  server.on("/trigger_sos", HTTP_GET, []() {
    Serial.println("SYSTEM EVENT: Virtual SOS Triggered via User Interface.");
  ...
  server.on("/simulate_alert", HTTP_GET, []() {
    Serial.println("SYSTEM EVENT: Seismic diagnostic simulated via User Interface.");
  ...
  server.begin();
  Serial.println("SYSTEM READY: Awaiting client connections on 'Pathfinder-Net'");
}
...
void loop() {
  ...
  if (hasMPU && (millis() - lastSensorRead > 10)) {
    ...
    if (abs(filtered_Z) > IMPACT_THRESHOLD && (millis() - lastAlertTime > 5000)) {
      Serial.println("SYSTEM EVENT: Genuine seismic impact detected.");
...
void sendLoRaAlert(String alertType) {
  if (hasLoRa) {
    ...
    LoRa.endPacket();
    Serial.println("LORA TX: Packet Transmitted -> " + alertType);
  } else {
    Serial.println("LORA TX (MOCK): " + alertType);
  }
}